...
public final class Combat {
	...
	public Combat(Unit unit, Enemy enemy){
		this.unit = unit;
		this.enemy = enemy;
	}
	...
	public float calculateDamage(Entry<Weapon, Byte> entry) {
		...
		val rules = _setRules(weapon);
		
		val hitRollDicePool = _hitRoll(rules, weapon, quantity);
		val woundRollDicePool = _woundRoll(enemy, weapon, rules, hitRollDicePool);
		val savingThrowDicePool = _savingThrows(woundRollDicePool, weapon, enemy, rules);
		return _assertDamage(savingThrowDicePool, weapon, enemy);
	}
	 ...
	 */
	private HitDicePool _hitRoll(CombatRules rules, Weapon weapon, byte quantity) {
		...
		return new HitDicePool(new DicePool(0,hits),lethalHits);
	}
	 ...
	 */
	private DicePool _woundRoll(Enemy enemy, Weapon weapon, CombatRules rules, HitDicePool hitRollDicePool) {
		...
		@var float probabilityToWound = _compare(strength, toughness);
	 ...
	 * migitate the damage
	 */
	private DicePool _savingThrows(DicePool woundRollDicePool, Weapon weapon, Enemy enemy, CombatRules rules) {
		...
		return new DicePool(0, missedSaves);
	}
	
	/**
	 * The damage step of the calculation.
	 */
	private float _assertDamage(DicePool savingThrowDicePool, Weapon weapon, Enemy enemy) {
	 ...
	 */
	private record DicePool(float total, float successes) {};
		
	/**
	 * This structure incorporates the main dice pool and extends it with
	 * the hit roll specific functionality
	 */
	private record HitDicePool(DicePool dicePool, float lethalHits) {};
	 ...
	 */
	private CombatRules _setRules(Weapon weapon) {
